...
export default function RaumplanerApp() {
  ...
  function handleApplyFloor() {
    ...
    setRendering(true);
    setCurrentStep(3);
    // FloorPreview component will handle the rendering
  }
    ...
    <div className="min-h-screen" style={{ backgroundColor: "var(--oak-bg)" }}>
      ...
      <header className="border-b bg-white" role="banner" style={{ borderColor: "var(--grey-lighter)" }}>
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6">
          ...
          <nav className="flex items-center gap-4" aria-label="Hauptnavigation">
            ...
            <a href="https://www.parkettboerse.net/shop" target="_blank" rel="noopener noreferrer" className="text-sm font-medium transition-colors hover:opacity-80" style={{ color: "var(--oak)" }}>
              Shop
            </a>
            <a href="https://www.parkettboerse.net/beratung" target="_blank" rel="noopener noreferrer" className="text-sm font-medium transition-colors hover:opacity-80" style={{ color: "var(--oak)" }}>
              Beratung
            </a>
          </nav>
        </div>
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8">
        ...
        <div
          ...
          style={{ boxShadow: "0 4px 6px -1px rgba(139, 105, 20, 0.08), 0 20px 40px -8px rgba(139, 105, 20, 0.12)" }}
        >
          <div className="flex flex-col lg:flex-row">
            ...
            <div className="flex-[65] border-b lg:border-b-0 lg:border-r" style={{ borderColor: "var(--grey-lighter)" }}>
              <div className="p-6 sm:p-8">
                ...
                {/* Step 2: Photo with floor detection status */}
                {currentStep === 2 && uploadedImage && (
                  <div className="animate-fadeIn">
                    <div className="relative w-full overflow-hidden rounded-xl shadow-lg">
                      <img src={uploadedImage} alt="Hochgeladenes Raumfoto" className="w-full object-contain" />
                      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 via-black/40 to-transparent px-5 pb-5 pt-14">
                        <div className="flex flex-wrap items-center justify-between gap-3">
                          <div className="flex items-center gap-2">
                            {analyzing ? (
                              <>
                                <div className="h-5 w-5 shrink-0 animate-spin rounded-full border-2 border-t-transparent" style={{ borderColor: "var(--oak-pale)", borderTopColor: "var(--white)" }} />
                                <span className="text-sm font-medium text-white">Boden wird erkannt...</span>
                              </>
                            ) : (
                              <>
                                <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full" style={{ backgroundColor: "var(--green)", color: "var(--white)" }}>
                                  <svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" />
                                  </svg>
                                </span>
                                <span className="text-sm font-medium text-white">
                                  <span className="hidden sm:inline">Boden erkannt — Wählen Sie rechts einen Bodenbelag aus →</span>
                                  <span className="sm:hidden">Boden erkannt</span>
                                </span>
                              </>
                            )}
                          </div>
                          <button onClick={handleReset} aria-label="Anderes Foto hochladen" className="rounded-lg bg-white/20 px-3 py-1.5 text-xs font-medium text-white backdrop-blur-sm transition-colors hover:bg-white/30">
                            Anderes Foto
                          </button>
                        </div>
                      </div>
                    </div>
                    <button onClick={scrollToSidebar} aria-label="Zum Bodenkatalog scrollen" className="mt-4 w-full rounded-xl py-2.5 text-center text-sm font-semibold text-white lg:hidden" style={{ backgroundColor: "var(--oak)" }}>
                      Böden anzeigen ↓
                    </button>
                  </div>
                )}

                {/* Step 3: Rendering (client-side) */}
                    ...
                    {error ? (
                      <div className="relative w-full overflow-hidden rounded-xl shadow-lg">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center rounded-xl">
                          <div className="mx-4 max-w-sm rounded-2xl bg-white px-8 py-8 text-center" style={{ boxShadow: "0 8px 32px rgba(0,0,0,0.12)" }}>
                            ...
                            <div className="mt-5 flex flex-col gap-2">
                              <button onClick={handleApplyFloor} className="rounded-lg py-2 text-sm font-semibold text-white" style={{ backgroundColor: "var(--oak)" }}>
                                Nochmal versuchen
                              </button>
                              <button onClick={handleTryAnother} className="rounded-lg border py-2 text-sm font-semibold" style={{ borderColor: "var(--grey-lighter)", color: "var(--dark)" }}>
                      ...
                      <div className="relative w-full overflow-hidden rounded-xl shadow-lg">
                        <img src={uploadedImage} alt="Vorschau wird erstellt" className="w-full object-contain" style={{ filter: "blur(2px)", opacity: 0.6 }} />
                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                          <div className="h-10 w-10 animate-spin rounded-full border-4 border-t-transparent" style={{ borderColor: "var(--oak-pale)", borderTopColor: "var(--oak)" }} />
                          <p className="mt-3 text-sm font-semibold" style={{ color: "var(--dark)" }}>Vorschau wird erstellt...</p>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="animate-fadeIn">
                    ...
                    <div className="mt-5 flex flex-col gap-3 sm:flex-row">
                      <button
                        ...
                        style={{ background: "linear-gradient(135deg, var(--oak) 0%, var(--oak-light) 100%)", boxShadow: "0 4px 12px rgba(139, 105, 20, 0.3)" }}
                      >
                      ...
                      <button onClick={handleTryAnother} className="flex flex-1 items-center justify-center rounded-xl border py-3 text-sm font-semibold transition-colors hover:bg-gray-50" style={{ borderColor: "var(--grey-lighter)", color: "var(--dark)" }}>
                        Anderen Boden testen
                      </button>
                      <button onClick={handleDownload} className="flex flex-1 items-center justify-center gap-2 rounded-xl py-3 text-sm font-semibold text-white transition-opacity hover:opacity-90" style={{ backgroundColor: "var(--green)" }}>
                ...
                {currentStep === 5 && selectedFloor && (
                  <div className="animate-fadeIn">
                    <ProductDetail product={selectedFloor} onBack={() => setCurrentStep(4)} />
                  </div>
                )}
              </div>
            </div>
            ...
            <div ref={sidebarRef} className="flex-[35] lg:max-h-[650px]">
              ...
              {currentStep === 3 && (
                <div className="pointer-events-none opacity-50">
                  <FloorCatalog products={products} loading={productsLoading} selectedFloor={selectedFloor} onFloorSelect={() => {}} onApply={() => {}} />
                </div>
              )}
              ...
              {currentStep === 5 && selectedFloor && resultImage && (
                <div className="flex h-full flex-col p-5 sm:p-6">
                  ...
                  <div className="overflow-hidden rounded-xl shadow-md">
                    <img src={resultImage} alt="Vorschau" className="w-full object-cover" />
                  </div>
                  <div className="mt-4 flex flex-col gap-2">
                    <button onClick={() => setCurrentStep(4)} className="rounded-lg border py-2 text-center text-sm font-semibold transition-colors hover:bg-gray-50" style={{ borderColor: "var(--grey-lighter)", color: "var(--dark)" }}>
                      Zurück zur Vorschau
                    </button>
                    <button onClick={handleTryAnother} className="rounded-lg py-2 text-center text-sm font-medium" style={{ color: "var(--oak)" }}>
                      Anderen Boden testen
                    </button>
                  </div>
                </div>
      ...
      {/* Hidden FloorPreview canvas — renders when step 3 + rendering */}
